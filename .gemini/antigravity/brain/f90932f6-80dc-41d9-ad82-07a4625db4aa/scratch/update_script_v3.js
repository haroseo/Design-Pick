...
// 2. script.js 내의 이벤트 초기화 리스너 추가 (colorSearchSort 변경 감지)
const searchInputListener = "this.colorInput?.addEventListener('input', () => this.searchColor());";
if (script.indexOf("colorSearchSort") === -1) {
...
// 3. searchColor() 함수의 경계를 정확히 색인하여 치환
// searchColor() { ... } 가 730줄 부근에 있습니다.
// 'searchColor() {' 의 위치를 잡은 뒤 다음 함수의 시작인 'adjustColor('의 위치를 찾아 그 사이를 덮어씁니다.
const searchColorStartIdx = script.indexOf('searchColor() {');
const adjustColorStartIdx = script.indexOf('adjustColor(step) {');

if (searchColorStartIdx !== -1 && adjustColorStartIdx !== -1) {
    ...
    script = script.substring(0, searchColorStartIdx) + searchColorReplacement + script.substring(adjustColorStartIdx);
    
    // 맨 뒤에 오토마타와 유사색 검출 함수 삽입
    script = script + "\n\n" + automataFunction;

    fs.writeFileSync(scriptPath, script, 'utf8');
    console.log('Successfully updated script.js without losing handleKeyPress function!');
} else {
    console.log('Error: searchColor or adjustColor index not found in script.js');
}
