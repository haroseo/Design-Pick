...
class ColorPalette {
    ...
    init() {
        ...
        document.getElementById('navMyBtn')?.addEventListener('click', () => this.switchTab('mypalettes'));
        ...
        document.getElementById('closeDetailBtn')?.addEventListener('click', () => this.closeColorDetail());
    ...
    handleRouting() {
        ...
        if (hash.startsWith('#view=')) {
            ...
            const detailView = document.getElementById('colorDetailView');
            if (detailView && detailView.classList.contains('active')) this.switchTab('colors');
        }
    }
    ...
    getColorWisdom(r, g, b) {
        ...
        if (h >= 0 && h < 30) {
            ...
            res.advice = isKr ? ['중요한 CTA 버튼에 사용해보세요.', '활동적인 스포츠 브랜드에 완벽합니다.'] : ['Try it for important CTA buttons.', 'Perfect for active sports brands.'];
            res.usage = isKr ? ['스포츠웨어', '식품 브랜드'] : ['Sportswear', 'Food Brands'];
        } else if (h >= 180 && h < 260) {
            res.tag = 'Trust & Calm';
            ...
            res.advice = isKr ? ['핀테크나 비즈니스 앱의 주조색으로 추천합니다.', '그레이 톤과 함께 쓰면 더 세련되어 보입니다.'] : ['Recommended as a primary color for fintech apps.', 'Looks more sophisticated with gray tones.'];
            res.usage = isKr ? ['뱅킹 앱', '테크 기업'] : ['Banking Apps', 'Tech Companies'];
        }
        return res;
    }

    shareCurrentColor() { const url = window.location.href; this.fallbackCopy(url, '공유 링크가 복사되었습니다!'); }
    ...
    loadStorage() {
        ...
        this.favorites = savedFavs.map(f => ({ hex: f.hex, name: f.name, isStarred: f.isStarred || false, category: f.category || this.getCategoryFromRgb(...Object.values(this.hexToRgb(f.hex))), createdAt: f.createdAt || Date.now() }));
    ...
    renderHistory() { const tr = document.getElementById('historyTray'); if (!tr) return; tr.innerHTML = this.history.length ? this.history.map(hex => `<div class="history-item" style="background:${hex}" onclick="app.setColorFromHex('${hex}')"></div>`).join('') : ''; }
    ...
    toggleFavorite() {
        ...
        else { const cat = this.getCategoryFromRgb(this.r, this.g, this.b); this.favorites.unshift({ hex, name, isStarred: false, category: cat, createdAt: Date.now() }); this.showToast('보관함에 저장되었습니다. (♥)'); this.playSuccessSound(); }
    ...
    renderFavorites() {
        const list = document.getElementById('myPalettesList'); const countBadge = document.getElementById('myPalCount'); if (!list) return;
        let filtered = this.favorites.filter(f => {
            ...
            const matchesFilter = this.activeFilter === 'all' || (this.activeFilter === 'starred' && f.isStarred) || (f.category === this.activeFilter);
            return matchesSearch && matchesFilter;
        });
        filtered.sort((a, b) => (b.isStarred ? 1 : 0) - (a.isStarred ? 1 : 0));
        if (countBadge) countBadge.textContent = this.favorites.length;
        const trans = uiTranslations[this.lang];
        if (filtered.length === 0) { list.innerHTML = `<div class="my-pal-empty" style="grid-column: 1/-1; padding: 60px; text-align: center; color: #999;"><p>${this.favorites.length === 0 ? trans.empty_fav : trans.search_result_empty}</p></div>`; return; }
        list.innerHTML = filtered.map(f => `<div class="my-pal-card ${f.isStarred ? 'is-starred' : ''}" onclick="app.selectMyColor('${f.hex}')"><div class="my-pal-color" style="background:${f.hex}"></div><div class="my-pal-info"><div class="my-pal-name">${f.name}</div><div class="my-pal-hex">${f.hex}</div></div><div class="card-actions" onclick="event.stopPropagation()"><button class="action-btn star-btn ${f.isStarred ? 'active' : ''}" onclick="app.toggleStar('${f.hex}')"><svg class="chip-icon"><use href="#icon-${f.isStarred ? 'star-filled' : 'star'}"/></svg></button><button class="action-btn delete-btn" onclick="app.deleteFavorite('${f.hex}')"><svg class="chip-icon"><use href="#icon-trash"/></svg></button></div></div>`).join('');
    }

    selectMyColor(hex) { this.setColorFromHex(hex); this.switchTab('picker'); }
    toggleStar(hex) { const item = this.favorites.find(f => f.hex === hex); if (item) { item.isStarred = !item.isStarred; this.saveLocalFavorites(); this.renderFavorites(); if (item.isStarred) this.playTickSound(); } }
    deleteFavorite(hex) { const idx = this.favorites.findIndex(f => f.hex === hex); if (idx !== -1) { this.favorites.splice(idx, 1); this.saveLocalFavorites(); this.updateFavBtnState(this.rgbToHex(this.r, this.g, this.b)); this.renderFavorites(); this.showToast('삭제되었습니다.'); } }
    getCategoryFromRgb(r, g, b) { const { h, s, l } = this.rgbToHsl(r, g, b); if (s < 12) return 'neutral'; if (h < 15 || h >= 345) return 'red'; if (h < 45) return 'orange'; if (h < 75) return 'yellow'; if (h < 165) return 'green'; if (h < 260) return 'blue'; if (h < 345) return 'purple'; return 'neutral'; }
    ...
    playSound(type) { if (this.volume <= 0) return; this.initAudio(); if (!this.audioCtx || this.audioCtx.state === 'suspended') return; const osc = this.audioCtx.createOscillator(); const gain = this.audioCtx.createGain(); osc.connect(gain); gain.connect(this.audioCtx.destination); const now = this.audioCtx.currentTime; if (type === 'tick') { osc.type = 'triangle'; osc.frequency.setValueAtTime(800, now); osc.frequency.exponentialRampToValueAtTime(100, now + 0.05); gain.gain.setValueAtTime(0.3 * this.volume, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.05); osc.start(now); osc.stop(now + 0.05); } else if (type === 'click') { osc.type = 'sine'; osc.frequency.setValueAtTime(1200, now); gain.gain.setValueAtTime(0.2 * this.volume, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.1); osc.start(now); osc.stop(now + 0.1); } else if (type === 'success') { osc.type = 'sine'; osc.frequency.setValueAtTime(500, now); osc.frequency.setValueAtTime(800, now + 0.1); gain.gain.setValueAtTime(0.2 * this.volume, now); gain.gain.linearRampToValueAtTime(0, now + 0.3); osc.start(now); osc.stop(now + 0.3); } }
    ...
    copyExport(type) { if (type === 'figma') { const size = 100; const colors = this.favorites.length ? this.favorites.map(f => f.hex) : [this.rgbToHex(this.r, this.g, this.b)]; const rects = colors.map((c, i) => `<rect x="${i*(size+20)}" y="0" width="${size}" height="${size}" fill="${c}" rx="12"/>`).join(''); this.fallbackCopy(`<svg width="${colors.length*(size+20)}" height="${size}" xmlns="http://www.w3.org/2000/svg">${rects}</svg>`, 'Figma SVG가 복사되었습니다.'); } else { const el = document.getElementById(type === 'css' ? 'exportCssCode' : type === 'tw' ? 'exportTwCode' : 'exportHexCode'); if (el) this.fallbackCopy(el.textContent, '코드가 복사되었습니다!'); } }
    ...
    searchColor() { const q = this.sanitizeInput(this.colorInput.value).toLowerCase(); if (!q) { this.searchResults.innerHTML = ''; return; } const res = []; for (const [name, color] of Object.entries(colorNameReferences)) { const nameEn = color.name_en || ''; if (name.toLowerCase().includes(q) || nameEn.toLowerCase().includes(q) || (color.tags || []).some(t => t.includes(q))) res.push({ name, color }); } this.searchResults.innerHTML = res.slice(0, 8).map(({ name, color }) => { const displayName = (this.lang === 'en' && color.name_en) ? color.name_en : name; return `<div class="search-result-item" onclick="app.setColorFromHex('${color.hex}')"><div class="search-result-color" style="background-color:${color.hex}"></div><div class="search-result-info"><div class="search-result-name">${displayName}</div><div class="search-result-hex">${color.hex}</div></div></div>`; }).join(''); }
    sanitizeInput(input) { const d = document.createElement('div'); d.textContent = input; return d.innerHTML.substring(0, 60); }
    handleKeyPress(e) { if (['INPUT', 'TEXTAREA'].includes(document.activeElement.tagName)) return; const activeId = document.querySelector('.tab-content.active')?.id; if (e.code === 'Space') { e.preventDefault(); if (activeId === 'inspiration') { if (this.reelRunning) this.stopInspirationReel(); else if (!this.reelDecelerating) { this.inspirationStopped = false; this.startInspirationReel(); } } else if (activeId === 'picker' && !this.isRouletting) this.startRoulette(); } else if (e.key === 'ArrowLeft') { e.preventDefault(); if (activeId === 'today') this.inspoPrev(); else if (activeId === 'picker' && !this.isRouletting) this.adjustColor(-1); } else if (e.key === 'ArrowRight') { e.preventDefault(); if (activeId === 'today') this.inspoNext(); else if (activeId === 'picker' && !this.isRouletting) this.adjustColor(1); } }
    ...
    initFeedbackUI() {
        const fab = document.getElementById('fabFeedback'); if (fab) fab.onclick = () => this.openFeedbackModal();
        const closeBtn = document.getElementById('closeFeedbackBtn'); if (closeBtn) closeBtn.onclick = () => document.getElementById('feedbackModal')?.classList.remove('show');
        const cancelBtn = document.getElementById('btnCancelFb'); if (cancelBtn) cancelBtn.onclick = () => document.getElementById('feedbackModal')?.classList.remove('show');
        const submitBtn = document.getElementById('btnSubmitFb');
        if (submitBtn) {
            submitBtn.onclick = async () => {
                const textEl = document.getElementById('fbText'); const rating = this.currentRating || 5; const text = textEl?.value;
                if (!text) return this.showToast('내용을 입력해주세요.');
                this.showToast('피드백이 전송되었습니다. 감사합니다!');
                document.getElementById('feedbackModal')?.classList.remove('show');
                if (textEl) textEl.value = '';
                if (this.supabase) await this.supabase.from('feedbacks').insert([{ rating: parseInt(rating), text, created_at: new Date() }]);
                else { const fbs = JSON.parse(localStorage.getItem('designpick_feedbacks') || '[]'); fbs.push({ rating: parseInt(rating), text, date: new Date() }); localStorage.setItem('designpick_feedbacks', JSON.stringify(fbs)); }
            };
        }
        const stars = document.querySelectorAll('.rating-star');
        stars.forEach(s => {
            s.onclick = () => {
                const val = s.dataset.rating; this.currentRating = val;
                stars.forEach(star => star.classList.toggle('active', parseInt(star.dataset.rating) <= parseInt(val)));
            };
        });
    }
    ...
    renderFonts() {
        const grid = document.getElementById('fontsGrid'); if (!grid || typeof designFonts === 'undefined') return;
        grid.innerHTML = designFonts.map(f => {
            if (!this.fontLoaded.has(f.id)) { const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = f.url; document.head.appendChild(link); this.fontLoaded.add(f.id); }
            const cat = this.lang === 'kr' ? f.category_kr : f.category; const note = this.lang === 'kr' ? f.note : f.note_en;
            const trans = uiTranslations[this.lang];
            return `<div class="font-card-v2" style="font-family: ${f.family}"><div class="font-card-header"><div class="font-info-meta"><span class="font-cat-badge">${cat}</span><h3 class="font-main-name">${f.name}</h3></div><button class="btn-font-copy" onclick="app.copyFontStack(\'${f.family}\')">${trans.font_copy_stack}</button></div><div class="font-preview-area"><p class="font-preview-text" data-original="${f.sample}">${f.sample}</p></div><div class="font-card-footer"><div class="designer-note-box"><span class="note-label">${trans.font_designer_note}</span><p class="note-content">${note}</p></div><div class="font-pairing-box"><span class="pair-label">${trans.font_pairing}:</span><span class="pair-value">${f.pair}</span></div></div></div>`;
        }).join('');
    }
    copyFontStack(family) { const stack = `font-family: ${family};`; navigator.clipboard.writeText(stack).then(() => this.showToast(this.lang === 'kr' ? '폰트 스택이 복사되었습니다.' : 'Font stack copied.')); }

    openAdminDashboard() { document.getElementById('adminAuthModal')?.classList.add('show'); }
    verifyAdmin() {
        ...
        if (pw === 'rgb') {
            ...
            if (modal) {
                this.renderAdminFeedbacks(); modal.classList.add('show'); this.showToast('실시간 동기화 활성화됨');
                if (this.supabase) {
                    this.feedbackChannel = this.supabase.channel('admin-feedbacks').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'feedbacks' }, () => { this.renderAdminFeedbacks(); this.showToast('새로운 피드백! 🔔'); }).subscribe();
                }
            }
        } else this.showToast('비밀번호가 틀렸습니다.');
    }
    async renderAdminFeedbacks() {
        ...
        let feedbacks = [];
        if (this.supabase) { const { data, error } = await this.supabase.from('feedbacks').select('*').order('created_at', { ascending: false }); if (!error) feedbacks = data; }
        else feedbacks = JSON.parse(localStorage.getItem('designpick_feedbacks') || '[]').reverse();
        if (feedbacks.length === 0) { list.innerHTML = '<div style="text-align:center; padding:40px; color:#888;">수집된 피드백이 없습니다.</div>'; return; }
