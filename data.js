...
// ─── Update: Academy Guides (Typography, Grid, UI Patterns, Portfolio) ──────
const typographyGuide = [
    {
        title: '타이포그래피의 핵심 요소',
        content: `1. <b>행간(Line-height)</b>: 줄과 줄 사이의 간격. 모바일에서는 보통 폰트 크기의 1.5~1.6배가 가독성이 좋습니다.<br>
        2. <b>자간(Letter-spacing)</b>: 글자 사이의 간격. 폰트 크기가 커질수록 자간은 좁게(-2%~-4%), 작아질수록 넓게 주어야 눈에 잘 띕니다.<br>
        3. <b>위계(Hierarchy)</b>: 제목, 소제목, 본문의 크기 차이를 확실하게 둡니다. 애매한 크기 차이는 오히려 시선을 분산시킵니다.`
    },
    {
        title: '명조체(Serif) vs 고딕체(Sans-serif)',
        content: `<b>명조체(Serif)</b>: 글자 끝에 장식이 있는 폰트. 전통적, 권위적, 우아함, 럭셔리한 느낌을 주며 종이 매체나 브랜딩에 적합합니다.<br>
        <b>고딕체(Sans-serif)</b>: 장식이 없는 폰트. 모던함, 깔끔함, 친근함, IT 테크 느낌을 주며 모바일 UI와 화면에 최적화되어 있습니다.`
    }
];
const gridLayoutGuide = [
    {
        title: '8px 그리드 시스템',
        content: `UI 디자인의 표준. 모든 여백(Margin, Padding)과 컴포넌트 크기를 8의 배수(8, 16, 24, 32, 40...)로 설계하는 방식입니다. 화면 해상도가 달라져도 가장 안정적인 비율을 유지합니다.`
    },
    {
        title: 'F-패턴과 Z-패턴',
        content: `사용자의 시선 이동 패턴.<br>
        - <b>F-패턴</b>: 텍스트가 많은 블로그나 기사에서 위에서 아래로, 좌에서 우로 읽는 형태.<br>
        - <b>Z-패턴</b>: 이미지가 많은 랜딩 페이지에서 지그재그로 시선이 이동하는 형태. 시선 끝에 콜투액션(버튼)을 배치하세요.`
    }
];
const uiPatternsGuide = [
    {
        title: '올바른 버튼(Button) 설계법',
        content: `1. <b>명확한 상태(States)</b>: Default, Hover, Active, Disabled 상태를 시각적으로 확실히 구분해야 합니다.<br>
        2. <b>터치 영역</b>: 모바일 화면에서 버튼의 최소 터치 영역은 44x44px 이상이어야 오타율이 적습니다.<br>
        3. <b>일관된 라운딩</b>: 앱 전체에서 버튼의 모서리 둥글기(Border-radius)가 동일해야 브랜드 통일성이 생깁니다.`
    }
];
const portfolioGuide = [
    {
        title: '합격하는 포트폴리오의 구조',
        content: `단순히 예쁜 이미지를 나열하는 것은 <b>'그림'</b>이지 <b>'디자인'</b>이 아닙니다.<br>
        - <b>문제 정의</b>: 왜 이 디자인을 시작했는가?<br>
        - <b>타겟 분석</b>: 누구를 위한 디자인인가?<br>
        - <b>해결 과정</b>: 컬러, 폰트, 레이아웃을 선택한 '논리적인 이유'<br>
        - <b>결과물</b>: 고퀄리티 목업과 프로토타입 시연`
    }
];
